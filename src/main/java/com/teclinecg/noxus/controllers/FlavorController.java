...
public class FlavorController {
    ...
    public ResponseEntity<Page<FlavorDto>> findAllFlavorsPaginated(Pageable pageable) {
        Page<FlavorDto> flavors = flavorService.findAllFlavorsPaginated(pageable);
    ...
    public ResponseEntity<FlavorDto> findFlavorById(@PathVariable Long id) {
        FlavorDto flavor = flavorService.findFlavorById(id);
        return ResponseEntity.ok().body(flavor);
    }

    @PostMapping
    @ApiOperation(value = "http://localhost:8080/flavor", notes = "Save a new flavor")
    public ResponseEntity<FlavorDto> saveFlavor(@Valid @RequestBody FlavorDto flavorDto) {
    ...
    public ResponseEntity<FlavorDto> deleteFlavorById(@PathVariable Long id) {
