...
public class DrinkController {
    ...
    public ResponseEntity<Page<DrinkDto>> findAllDrinksPaginated(Pageable pageable) {
        Page<DrinkDto> drinks = drinkService.findAllDrinksPaginated(pageable);
    ...
    public ResponseEntity<DrinkDto> findDrinkById(@PathVariable Long id) {
        DrinkDto drink = drinkService.findDrinkById(id);
        return ResponseEntity.ok().body(drink);
    }

    @PostMapping
    @ApiOperation(value = "http://localhost:8080/drink", notes = "Save a new drink")
    public ResponseEntity<DrinkDto> saveDrink(@Valid @RequestBody DrinkDto drinkDto) {
    ...
    public ResponseEntity<DrinkDto> deleteDrinkById(@PathVariable Long id) {
